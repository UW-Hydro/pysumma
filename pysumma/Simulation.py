...
class Simulation(object):
    def __init__(self, file_manager_path):
        self.file_manager = FileManager(filepath_manager_path)
...
class FileManager(object):
    ...
    fileman_opts = ['header', 'settings_path', 'input_path', 'output_path',
    ...
    def read(self, path):
        ...
        for linum, line in enumerate(self.original_contents):
            if not line.startswith('!'):
                attrs[fileman_opts[opt_count]] = line.split('!')[0].strip()
                opt_count += 1
                self.overwrite_lines.append(linum)
        assert opt_count == len(fileman_opts)
        self.__dict__.update(attrs)

    def write(self, path=None):
        assert original_file_contents is not None
        if not path:
        ...
        for linum, line in enumerate(self.original_contents):
            if linum in self.overwrite_lines:
                ...
                newval = self.__dict__[fileman_opts[opt_count]]
