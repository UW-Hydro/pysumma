...
class Plotting:
	...
	def plot_1d(self, variable_num):
		...
		ax = self.ds[var_list_key].plot()
		plt.ylabel('{} ({})'.format(self.ds[var_list_key].long_name, self.ds[var_list_key].units))
		plt.show()
		return ax
	...
	def plot_1d_hru(self, hru_num, variable_num):
		var_dict= dict(self.ds.data_vars)
		var_list_key = list(var_dict.keys())[variable_num]
		ax = self.ds[var_list_key][:,hru_num].plot()
		plt.ylabel('{} ({})'.format(self.ds[var_list_key].long_name, self.ds[var_list_key].units))
		plt.show()
		return ax
	...
	def plot_1d_layer(self, hru_num, variable_num_Y, start_index_X, timestep):
		...
		ax = self.ds[var_list_key_Y][:,hru_num].plot()
		...
		plt.ylabel('{} ({})'.format(self.ds[var_list_key_Y].long_name, self.ds[var_list_key_Y].units))
		plt.show()
		return ax
	...
	def plot_2d(self, hru_num, variable_num_Y, variable_num_X, midto, timesteps):
		...
		for timestep in timesteps:
			...
			# - 1 since the SUMMA indices are 1-based and python indices are 0-based
			start_index = self.ds[var_list_key][timestep].values[0] - 1
			endIndex = start_index + layers
			ax = plt.plot(self.ds[var_list_key_X][start_index:endIndex],
							self.ds[var_list_key_Y][start_index:endIndex,0],
							label='t = {: 2d}'.format(timestep+1))
		plt.ylabel('{} ({})'.format(self.ds[var_list_key_Y].long_name, self.ds[var_list_key_Y].units))
		plt.xlabel('{} ({})'.format(self.ds[var_list_key_X].long_name, self.ds[var_list_key_X].units))
		plt.legend(loc=3)
		plt.show()
		return ax
