...
class Simulation(object):
    ...
    def __init__(self, case_data, summa_code=None):
        ...
        self.decision_obj = Decisions(self.base_dir + '/settings/'
                                      + self.decision_path.value)

        if summa_code is None:
            self.modeloutput_obj = ModelOutput(
                self.base_dir + '/settings/' + self.OUTPUT_CONTROL.value,
                self.base_dir + '/settings/meta/var_lookup.f90')
            self.summa_code = summa_code
        else:
            self.summa_code = summa_code
            self.modeloutput_obj = ModelOutput(
                self.base_dir + '/settings/' + self.OUTPUT_CONTROL.value,
                summa_code + '/build/source/dshare/var_lookup.f90')
...
class FileManagerOption(Option):
    ...
    # By default, delimiter=None, but can be set to
    # split each line on different characters
    def __init__(self, parent, name):
        super().__init__(name, parent, key_position=1,
                         value_position=0, delimiter="!")

    # get value to change file manager value (value : a line divides
    # by delimiter "!", and directory and filename are value)
    @property
    def value(self):
